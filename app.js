...
// Real qiymətin üstünə köhnə baha qiymət göstərmək üçün
...
/* =========================
   PROFILE MODAL
========================= */
const PROFILE_KEY = "garnet_profile_v1";
...
function updateMiniProfileUI() {
  const profile = loadProfile();
  const box = $("#miniProfile");
  const text = $("#miniProfileText");
  ...
  text.textContent = `@${profile.instagram}`;
...
function setupProfileModal() {
  ...
  if (!saved || !saved.instagram || !saved.phone) {
    openProfileModal();
  } else {
    updateMiniProfileUI();
  }
  ...
  $("#saveProfileBtn")?.addEventListener("click", () => {
    let instagram = ($("#profileInstagram")?.value || "").trim();
    let phone = ($("#profilePhone")?.value || "").trim();

    instagram = instagram.replace("@", "").replace(/\s+/g, "");

    if (!instagram || !phone) {
      alert("Zəhmət olmasa Instagram adınızı və əlaqə nömrənizi qeyd edin.");
      return;
    }

    saveProfile({ instagram, phone });
    updateMiniProfileUI();
    closeProfileModal();
  });
  ...
  $("#profileModalBack")?.addEventListener("click", (e) => {
    if (e.target.id === "profileModalBack") {
      const savedProfile = loadProfile();
      if (savedProfile && savedProfile.instagram && savedProfile.phone) {
        closeProfileModal();
      }
    }
  });
...
function resetModalSelections() {
  ...
  if ($("#mSize")) $("#mSize").textContent = "Seç";

  if (current && $("#mPrice")) {
    $("#mPrice").innerHTML = `
      <span class="modalNewPrice">${money(current.price20)}</span>
      <span class="modalOldPrice">${money(oldPrice(current.price20))}</span>
    `;
  }
}
...
function updateModalPrice() {
  ...
  const newP = priceBySize(current.price20, size);
  const oldP = oldPrice(newP);

  if ($("#mPrice")) {
    $("#mPrice").innerHTML = `
      <span class="modalNewPrice">${money(newP)}</span>
      <span class="modalOldPrice">${money(oldP)}</span>
    `;
  }
}
...
function bindModalOptionHandlersOnce() {
  ...
  if (sizesWrap && !sizesWrap.dataset.bound) {
    ...
    sizesWrap.addEventListener("click", (e) => {
      ...
      if ($("#mSize")) $("#mSize").textContent = `${pick.size}ml`;
      $("#mDeliveryWrap")?.classList.remove("hidden");
...
function cartWhatsAppText() {
  ...
  if (profile?.phone) lines.push(`Əlaqə nömrəsi: ${profile.phone}`);
...
function getLiveViewMap(products) {
  ...
  const showCount = Math.max(2, Math.floor(total * 0.18));
  const shuffled = [...products].sort(() => Math.random() - 0.5);
  const selected = shuffled.slice(0, showCount);

  selected.forEach((p) => {
    map[p.id] = Math.floor(Math.random() * 3) + 1;
  });
  ...
  if (list.length === 0) {
    grid.innerHTML = `
      <div class="emptyState">
        Axtarışına uyğun məhsul tapılmadı.
      </div>
    `;
    return;
  }
    ...
    <div class="card">
      <div class="cTop">
        ...
        <div class="priceBadge">
          <span class="oldPrice">${money(oldPrice(p.price20))}</span>
          <span class="newPrice">${money(p.price20)}</span>
        </div>
...
function openModal(id) {
  ...
  if ($("#mName")) $("#mName").textContent = `${p.brand} — ${p.name}`;
  if ($("#mNotes")) $("#mNotes").textContent = p.notes || "";
  if ($("#mDesc")) $("#mDesc").textContent = p.desc || "";
  if ($("#mCat")) $("#mCat").textContent = catLabel(p.cat);

  if ($("#mPrice")) {
    $("#mPrice").innerHTML = `
      <span class="modalNewPrice">${money(p.price20)}</span>
      <span class="modalOldPrice">${money(oldPrice(p.price20))}</span>
    `;
  }
...
function waText(p, sizeMl, delivery, finalPrice) {
  ...
  const msg =
    ...
    `${profile?.phone ? `• Əlaqə nömrəsi: ${profile.phone}\n` : ""}` +
...
========================= */
const fakeOrders = [
  "Na*** Afnan 9PM sifariş etdi",
  "El*** Bleu de Chanel sifariş etdi",
  "Ka*** Dior Sauvage sifariş etdi",
  "Mu*** Al-Rehab Dalal sifariş etdi",
  "Ra*** Baccarat Rouge sifariş etdi",
  "Ay*** Burberry Men sifariş etdi"
];
...
function showOrderPopup() {
  ...
  popup.innerHTML = `💬 ${randomOrder}<br><small>bir neçə saniyə əvvəl</small>`;
