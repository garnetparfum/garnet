// =========================
// Garnet Parfum - app.js (FINAL STABLE + CART)
// =========================

const WHATSAPP_PHONE = "994517238896"; // 0517238896

// ✅ 20ml baza qiymət → 30/50 avtomatik yuvarlaqlanır
function roundAzN(x){ return Math.round(x * 2) / 2; } // 0.5 AZN addımı
...
function requireSelectionsOrWarn(){
  if(!pick.size){ alert("Zəhmət olmasa həcm seçin: 20ml / 30ml / 50ml"); return false; }
  if(!pick.delivery){ alert("Zəhmət olmasa çatdırılma seçin: Metro / Taksi"); return false; }
  return true;
}
...
// ✅ 1 dəfəlik event delegation (düymələr 100% işləyəcək)
function bindModalOptionHandlersOnce(){
  ...
  if(delWrap && !delWrap.dataset.bound){
    ...
    delWrap.addEventListener("click", (e) => {
      ...
      if(!pick.size){ alert("Əvvəlcə həcm seçin (20/30/50ml)."); return; }
...
function addToCart(p, sizeMl, delivery){
  ...
  if(found) found.qty += 1;
    ...
    key,
    id:p.id, brand:p.brand, name:p.name, img:p.img,
    gender:p.gender, cat:p.cat, size, delivery, price, qty:1
  });
    ...
    <div style="display:flex; gap:10px; align-items:center; padding:10px; border:1px solid rgba(0,0,0,.08); border-radius:14px; background:rgba(255,255,255,.65);">
      ...
      <div style="flex:1;">
        ...
        <div style="display:flex; gap:8px; align-items:center; margin-top:8px;">
          ...
          <button class="btn ghost" data-remove="${i.key}"
                  style="margin-left:auto; padding:8px 10px; border-color:rgba(160,18,61,.35); color:#a0123d;">
            Sil
          </button>
        </div>
      </div>
  ...
  grid.innerHTML = list.map(p => `
    <div class="card">
      ...
      <div class="cBody">
        ...
        <div class="cBtns">
          <button class="sbtn primary" data-open="${p.id}">Seç</button>
        </div>
      </div>
    </div>
  `).join("");
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  // ✅ Modal "Səbətə əlavə et" — istədiyin qədər əlavə olunur
  $("#mCopy") && ($("#mCopy").onclick = () => {
    if(!current) return;
    if(!requireSelectionsOrWarn()) return;
